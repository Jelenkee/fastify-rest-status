...
module.exports = async function (log, instance, opts) {
    ...
    if (typeof config === "object") {
        ...
        instance.get(opts.prefix + CONFIG_PATH + "/:key", (req, rep) => {
            send(rep, { [req.params.key]: getConfigValue(req.params.key) });
        });
        ...
        instance.put(opts.prefix + CONFIG_PATH + "/:key", (req, rep) => {
            try {
                const val = req.query.value || getJSONBody(req).value;
                if (!(req.params.key in configObject)) {
                    sendError(rep, 400, "Adding new config values during runtime is not allowed.");
                    return;
                }
