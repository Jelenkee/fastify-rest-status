...
module.exports = async function (log, instance, opts) {
    if (opts.monitor) {
        ...
        const keys = Object.keys(metrics).filter(k => typeof metrics[k] === "function");
        const results = {};
        ...
        instance.get(opts.prefix + MONITOR_PATH + "/:key/:seconds", {
            ...
            try {
                send(rep, getMetric(req.params.key, seconds, true))
            } catch (error) {
        ...
        function getMetric(key, seconds, object) {
            seconds |= 0;
            ...
            if (seconds <= 0) {
                ...
                return object ? { min: val, max: val, avg: val } : val;
            }
        ...
        instance.decorate("getMetric", (key, seconds) => getMetric(key, seconds, true));
