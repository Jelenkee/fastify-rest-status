const { getJSONBody, sendSuccess, sendError, send } = require("./utils");
...
module.exports = async function (log, instance, opts) {
    ...
    function addLoggerRoutes(logger) {
        ...
        instance.get(opts.prefix + LOG_LEVEL_PATH + "/levels", (req, rep) => {
            send(rep, Object.values(logger.levels.labels));
        });
        instance.put(opts.prefix + LOG_LEVEL_PATH, (req, rep) => {
            try {
                const newLevel = req.query.level || getJSONBody(req).level;
                if (newLevel) {
                    logger.level = logger.levels.labels[newLevel] || newLevel;
                    log.info(`Log level set to ${logger.level}`);
