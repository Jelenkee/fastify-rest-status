const { sendSuccess, sendError, send, convertToAsync, errorToObject, createStore, ErrorWithCode } = require("./utils");
...
module.exports = async function (log, instance, opts) {
    if (opts.cron) {
        ...
        const persistentStore = await createStore(opts.cron);
        ...
        async function getCronjobList() {
            return persistentStore.entries()
                .then(entries => entries.map(entry => { const o = JSON.parse(entry[1]); delete o.history; return populateRunning(o); }))
                .catch(e => { throw new ErrorWithCode(`Error while fetching cronjobs: ${e.message}`, 500); });
        }
